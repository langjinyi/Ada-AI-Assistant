...
def mount_app_routes(app: FastAPI, run_mode: str = None):
    ...
    # Tag: voice and text chat
    @app.post("/voice_text/chat",
              tags=["Voice and Chat"],
              summary="与llm模型语音对话(通过LLMChain)",
              )
    async def voice_chat(audio_file: UploadFile = File(None, description="用户输入语音文件"),
        ...
        if audio_file is not None:
            ...
            async def process_audio_and_transcribe(audio_io):
                ...
                input_features = v2t_processor(audio, sampling_rate=16000, return_tensors="pt").input_features.to(
                    "cuda")
            ...
            # Wait for audio processing and transcription to complete
        ...
        async def tts_infer(text_):
            ...
            return tts.infer(text_, stream=stream, params_infer_code=params_infer_code,
                             do_text_normalization=True, use_decoder=True)

        # Create a task for TTS inference
